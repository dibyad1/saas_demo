view: salesforce_user {
  ...
  dimension: name {
#     label: "{% if _view._name =='csm' %}Customer Success Manager{% elsif _view._name =='sales_manager' %} Sales Manager {% else %} Account Executive {% endif %}"

    label: "
          {% if _user_attributes['locale'] =='en' %}
            {% if _view._name =='csm' %}
              Customer Success Manager
            {% elsif _view._name =='sales_manager' %}
              Sales Manager
            {% else %}
              Account Executive
            {% endif %}
          {% elsif _user_attributes['locale'] =='ja_JP' %}
            {% if _view._name =='csm' %}
               カスタマーサクセスマネージャー
            {% elsif _view._name =='sales_manager' %}
              セールスマネージャー
  ...
  dimension: segment_region {
    ...
    action: {
      ...
      icon_url: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Google_Calendar_icon_%282020%29.svg/512px-Google_Calendar_icon_%282020%29.svg.png"
      form_param: {
  ...
  dimension: self_generated_ops_goals {
    label: "Self Generated Ops Goals"
    group_label: "Individual Goals"
